...
| **Sales Agents / Channels** | Key feature unavailable -- cannot generate illustrations for one product line, or new business submission is slow to the point of being unusable during peak hours. Other functions work. | Productivity severely reduced. Some products or channels are effectively blocked. Agents may lose sales because customers will not wait. |
...
| **Sales Agents / Channels** | Minor system glitch. Can still sell policies and submit applications. One supporting tool not working (commission report, client listing display). | Cannot access one supporting function. Core sales workflow unaffected. |
...
| **Sales Agents / Channels** | Cosmetic issue or minor annoyance. Does not affect ability to sell or service customers. | Nothing material. |
...
| **Reputational Damage** | Policyholders who cannot access services lose trust. Agents who cannot sell lose confidence in the platform. Word spreads through agency networks fast. In digital channels, social media amplifies negative experiences. | Customer churn is difficult to measure in real-time but compounds. An agent who loses a sale due to system downtime remembers it during the next contract renewal discussion. |
...
| **Customer Experience Degradation** | Customers and agents can work around the issue but with friction. Satisfaction scores decline. |
...
| **P1** | Sales Agents / Channels | Agent notification system, WhatsApp/Zalo broadcast, email | Within **30 minutes** | What is down, workaround if any, when next update will come | IC + Channel Management |
...
| **P2** | Agents (if agent-facing system affected) | Agent notification channel | Within **1 hour** | What is affected, workaround, expected resolution | Channel Management |

#### Communication Anti-Patterns

| Anti-Pattern | Why It Is Harmful | Do This Instead |
|---|---|---|
| "We are investigating" with no follow-up | Stakeholders assume you have forgotten. Agents call IT directly, disrupting the response. | Set a specific next-update time and honor it. |
| Technical jargon in customer/agent communications | "DB connection pool exhausted" means nothing to an agent with a customer waiting. | "The policy system is temporarily unavailable. We expect it back within [time]." |
...
| Sending technical detail to non-technical audiences | Sending a technical RCA to agents or executives. | Tailor the message to the audience. Technical detail for tech teams. Impact and timeline for business. |
...
The current priority matrix does not account for when the incident occurs. A P2 incident at 2 PM on a Tuesday (peak sales hours, agents in branches with customers) is materially different from the same incident at 11 PM on a Saturday. Recommendation:
...
Life insurance operations are heavily batch-dependent (premium allocation, commission calculation, regulatory reporting, reinsurance ceding). The scenarios in Section 8 (Tier 3) cover this well. However, the definitions and general indicators in Sections 3.1-3.4 do not explicitly call out batch window failures. Missing a nightly premium allocation batch means the next day's finance reconciliation is wrong, agents' commission statements are delayed, and downstream processes cascade. Recommendation: add batch window breach as an explicit indicator in the P2 general indicators.
...
4. **Agent licensing and compliance.** If the incident affects agent licensing systems, commission calculations, or compliance reporting for the sales force, the regulatory implications extend beyond IT into distribution compliance.
